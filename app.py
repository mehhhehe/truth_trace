...
#     This subclass disables the built-in Google Drive download step and
...
#         missing = [
#             fname for fname in self.raw_file_names
...
#     train_dataset = LocalUPFD(root=root, name=dataset_name,
#                               feature=feature_type, split="train")
#     val_dataset = LocalUPFD(root=root, name=dataset_name,
#                             feature=feature_type, split="val")
#     test_dataset = LocalUPFD(root=root, name=dataset_name,
#                              feature=feature_type, split="test")
...
#         """
#         Return metadata for a single news cascade:
#           - id
#           - numeric label
#           - label_name ("fake"/"real")
#           - optional text (if attached to the Data object as `text`)
#         """
...
#         # UPFD convention: 1 = fake, 0 = real (if your training used the opposite,
#         # flip this mapping accordingly).
...
#             elif probs.ndim == 1:
#                 # Already a single probability per sample
#                 baseline_prob = float(probs[0])
#             else:
#                 # Extreme edge case: just take the first value
...
#         # Graph label
#         label_int = int(data.y.item())
#         # Adjust mapping if in your training 1/0 means the opposite
#         label_name = "fake" if label_int == 1 else "real"

#         # Optional text (will be None for plain UPFD unless you attached .text earlier)
#         text = getattr(data, "text", None)

#         # Node features: first K dims to avoid huge JSON
#         K = 8
